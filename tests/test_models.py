...
_MAIN_TOOL_TYPES = ["2", "3", "4", "5", "8", "9", "10", "11", "13",
...
def test_registry_shape_and_tool_types():
    ...
    assert len(set(main)) == 18, "同一 toolType 不得注册两个模型名（重复编号见 NOT_REGISTERED）"
...
def test_absences_are_documented():
    ...
    assert any("28~34" in k for k in NOT_REGISTERED), "重复编号族必须有说明"
...
def test_availability_reason_mentions_legacy_only_when_mapped():
    unmapped = lookup("wenxin:reimagine")
    assert "BAIDU_LEGACY" not in availability(unmapped, allow_unverified=False)[1]
    mapped = lookup("wenxin:dewatermark")
    assert "BAIDU_LEGACY" in availability(mapped, allow_unverified=False)[1]
